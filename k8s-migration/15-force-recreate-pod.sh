...
OLD_POD=$(kubectl get pods -n ess -l app.kubernetes.io/name=your_livekit_api_key_here-authorisation-service -o jsonpath='{.items[0].metadata.name}')
...
# Получить новый pod
NEW_POD=$(kubectl get pods -n ess -l app.kubernetes.io/name=your_livekit_api_key_here-authorisation-service -o jsonpath='{.items[0].metadata.name}')
echo ""
