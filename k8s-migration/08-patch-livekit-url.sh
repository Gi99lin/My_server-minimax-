...
INTERNAL_SFU_URL="ws://ess-your_livekit_api_key_here-sfu.ess.svc.cluster.local:7880"

echo "=== Исправление LIVEKIT_URL через kubectl patch ==="
echo ""
echo "Текущая проблема:"
kubectl describe deployment -n $NAMESPACE ess-your_livekit_api_key_here-authorisation-service | grep "LIVEKIT_URL"
echo ""
...
spec:
  template:
    spec:
      containers:
      - name: your_livekit_api_key_here-authorisation-service
        env:
        - name: LIVEKIT_URL
          value: "$INTERNAL_SFU_URL"
        - name: LIVEKIT_KEY
          value: "your_livekit_api_key_here"
...
kubectl patch deployment -n $NAMESPACE ess-your_livekit_api_key_here-authorisation-service \
...
kubectl rollout status deployment/ess-your_livekit_api_key_here-authorisation-service -n $NAMESPACE

echo ""
echo "=== Проверка результата ==="
echo ""
kubectl describe deployment -n $NAMESPACE ess-your_livekit_api_key_here-authorisation-service | grep -A 5 "Environment:"
...
clients:
  - client_id: 0000000000000000000SYNAPSE
    ...
    client_secret: your_mas_client_secret_here
...
matrix:
  ...
  secret: your_mas_client_secret_here
...
echo ""
echo "   kubectl logs -n $NAMESPACE -l app.kubernetes.io/name=your_livekit_api_key_here-authorisation-service -f"
echo ""
