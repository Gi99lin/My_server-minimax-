...
kubectl get deployment -n ess ess-your_livekit_api_key_here-authorisation-service -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name=="LIVEKIT_URL")].value}'
echo ""
echo ""

# 5. Проверяем внутренний SFU
echo "[5] Внутренний LiveKit SFU (ClusterIP):"
kubectl get svc -n ess ess-your_livekit_api_key_here-sfu -o wide
echo ""

# 6. Проверяем Authorization Service
echo "[6] Authorization Service (NodePort):"
kubectl get svc -n ess ess-your_livekit_api_key_here-authorisation-service -o wide
echo ""
