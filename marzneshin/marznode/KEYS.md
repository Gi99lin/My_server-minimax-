# VPN Node Key Reference
# ⚠️ SENSITIVE — do not commit to public repos!
# Generated: 2026-03-01

## Node: NL-1 (Meppel, 85.208.110.9)
- **SNI**: www.bol.com
- **ASN**: AS209847 WorkTitans B.V.
- **Reality PrivateKey**: `aHYpapRPExAVUu7FAFrtQLkr33Y4_QqCzE-BendP-Fg`
- **Reality PublicKey**: `uv0nhtNTdsHmFYoKFGMd3vO8JtpkKCf9yYf36wiH1jM`
- **ShortIds**: `9e5fa93786fe035a`, `816506ea`, `da0e`, `af`
- **Hysteria2 obfs password**: `pWjNhuFZynQD2vT9rvlExsHxf_2of44t`

## Node: NL-2 (Amsterdam, 194.60.132.37)
- **SNI**: www.ns.nl
- **ASN**: AS216071 Servers Tech FZCO
- **Reality PrivateKey**: `yGP2lWOV8k_sCQSxmhWNJR3Q2I7lDQHi8pQmIv7hZ1Q`
- **Reality PublicKey**: `KW0_1AlQW752_llEAOA_hiMK6GGp4JHS-bSRCjnctzE`
- **ShortIds**: `e50487e1e0781312`, `7128c7b9`, `a5c6`, `63`
- **Hysteria2 obfs password**: `sjMFGJGNsJiKo3xyupNC2CgSvWzXx3a-`

## Node: FI (Helsinki, 185.231.206.53)
- **SNI**: www.helsinki.fi
- **ASN**: AS209847 WorkTitans B.V.
- **Reality PrivateKey**: `0CKCtV3adezK-aW-SdGxx3eOmfvbsMEuDShbBeFy7Hw`
- **Reality PublicKey**: `lgvlY3Vh-kzRAqllq6K0_GZPW1CoGPsaUuqFn2_JSmc`
- **ShortIds**: `7f2af75868d1223a`, `a4145fcd`, `61ba`, `86`
- **Hysteria2 obfs password**: `E7yPj1K5CNQbq04E6HZBpD5uu5Mp1eIC`

## Node: LU (Luxembourg, 45.12.139.177)
- **SNI**: www.nvidia.com
- **ASN**: AS209847 WorkTitans B.V.
- **Reality PrivateKey**: `8NnxRTQWzTCrdOD2s7nsH3cpWFzC5hib9Ri9-IL49HE`
- **Reality PublicKey**: `MiNV09NcFkyh097KVeTMkDtiXvmVef0Artaz1-rjtj4`
- **ShortIds**: `a445b412b415ec41`, `ab873cb7`, `7c16`, `ab`
- **Hysteria2 obfs password**: `LFltj6ZixEhM2zQH0JofWPPaCSGSk8Hy`

## Node: KZ-2 (Kazakhstan 2, 212.116.240.211)
- **SNI**: ge.gigglin.tech (via kz config)
- **Reality PrivateKey**: `ALN6rETOXROROAnizzLS_0hBC_2i4j1_wcbdXgYBiW0`
- **Reality PublicKey**: `lunMjXPoenYQnDS3ukqZ1G3ZpRCd7fwP74vt9XvjXWs`
- **ShortIds**: `4948e9b4649bb6d5`
- **Hysteria2 obfs password**: `vQWcsRO7hr7GNbLmq34bo5OSdj3+QQOn`

## Node: HOME (Home Server)
- **SNI**: www.nvidia.com
- **Ports**: VLESS Reality = `8443`, Hysteria2 = `10443`
- **Reality PrivateKey**: `aD6j_Ti6aWHFepe5ZcaSCplOxqnjJNGmVa-miTW3wHo`
- **Reality PublicKey**: `zyLqE_scOvPhm4ZHbPoUNeywmnQf-_UTCiIugpQlP2A`
- **ShortIds**: `99d1ee3d8620a069`, `cd2d6dbc`, `8051`
- **Hysteria2 obfs password**: `uETBP6tUHVT9i095Eqnam_ZMHI2L37Pj`
- **⚠️ LAN access enabled** — приватные IP (192.168.x.x) не блокируются
